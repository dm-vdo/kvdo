 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/uds-releases/krusty/src/uds/indexSession.c#18 $
 */
...
{
  const struct session_stats *sessionStats = &indexSession->stats;
...
int uds_suspend_index_session(struct uds_index_session *session, bool save)
{
  ...
  while (session->state & IS_FLAG_CLOSING) {
    wait_cond(&session->requestCond, &session->requestMutex);
  }
