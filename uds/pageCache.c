		    ...
		    unsigned int zoneCount)
{
  ...
  for (i = 0; i < cache->numCacheEntries; i++) {
    ...
    result = initialize_volume_page(geometry, &page->cp_pageData);
    if (result != UDS_SUCCESS) {
      return result;
    }
    clearPage(cache, page);
...
void freePageCache(PageCache *cache)
{
  ...
  if (cache->cache != NULL) {
    unsigned int i;
    for (i = 0; i < cache->numCacheEntries; i++) {
      destroy_volume_page(&cache->cache[i].cp_pageData);
    }
  }
  FREE(cache->index);
  FREE(cache->cache);
...
size_t getPageCacheSize(PageCache *cache)
{
  ...
  return sizeof(struct delta_index_page) * cache->numCacheEntries;
}
