 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/base/vdoSuspend.c#17 $
 */
...
static void suspendCallback(struct vdo_completion *completion)
{
  ...
  case SUSPEND_PHASE_LOGICAL_ZONES:
    drain_logical_zones(vdo->logicalZones, vdo->adminState.state,
                        reset_admin_sub_task(completion));
    return;

  case SUSPEND_PHASE_BLOCK_MAP:
    drain_block_map(vdo->blockMap, vdo->adminState.state,
                    reset_admin_sub_task(completion));
    return;

  case SUSPEND_PHASE_JOURNAL:
    drain_recovery_journal(vdo->recoveryJournal, vdo->adminState.state,
                           reset_admin_sub_task(completion));
    return;

  case SUSPEND_PHASE_DEPOT:
    drain_slab_depot(vdo->depot, vdo->adminState.state,
                     reset_admin_sub_task(completion));
    return;
