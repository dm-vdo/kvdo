 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/vdo-releases/magnesium-rhel7.5/src/c++/vdo/base/vdoLoad.c#1 $
 */
...
static int decodeVDO(VDO *vdo, bool validateConfig)
{
  ...
  if (upgradeRequired(vdo)) {
    if (requiresRebuild(vdo)) {
      ...
      logErrorWithStringError(result, "Cannot upgrade a dirty VDO.");
      return result;
    }

    // Do any online upgrade stuff here.
  }
