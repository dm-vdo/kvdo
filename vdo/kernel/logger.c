 ...
 * @param module_name  The name of the module doing the logging
			     ...
			     const struct va_format *vaf2)
{
	if (in_interrupt()) {
		printk("%s%s[%s]: %s%pV%pV\n", level, module_name,
		       getCurrentInterruptType(), prefix, vaf1, vaf2);
		return;
	}
	...
	if (((current->flags & PF_KTHREAD) != 0) &&
	    (strncmp(module_name, current->comm, strlen(module_name)) == 0)) {
		/*
		 * It's a kernel thread starting with "kvdo" (or whatever).
		 * Assume it's ours and that its name is sufficient.
		 */
		printk("%s%s: %s%pV%pV\n",
		       level,
		       current->comm,
		    ...
		    va_list args2)
{
	...
	emit_log_message(priorityToLogLevel(priority),
			 THIS_MODULE->name,
			 prefix,
			 &vaf1,
			 &vaf2);
...
/**********************************************************************/
int vLogWithStringError(int priority, int errnum, const char *format,
			va_list args)
{
