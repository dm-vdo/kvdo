 ...
 * 02110-1301, USA. 
 *
 * $Id: //eng/linux-vdo/src/c++/vdo/kernel/kvdoFlush.c#40 $
 */
 ...
 **/
static void kvdo_flush_work(struct vdo_work_item *item)
{
...
{
	struct vdo *vdo = &kvdo_flush->layer->vdo;
...
void vdo_complete_flush(struct vdo_flush **flush_ptr)
{
	...
	setup_work_item(&kvdo_flush->work_item,
			...
			NULL,
			BIO_Q_ACTION_FLUSH);
	enqueue_bio_work_item(kvdo_flush->layer->vdo.io_submitter,
			      &kvdo_flush->work_item);
	*flush_ptr = NULL;
}
